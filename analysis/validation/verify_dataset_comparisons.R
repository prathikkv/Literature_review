...
cat("SEARCH: VERIFYING DATASET COMPARISONS\n")
cat("===============================\n\n")

# Load the CAMK analysis results to see current comparisons
camk_results <- readRDS("output/CAMK_focused_analysis_results_corrected.rds")

cat("DATA: Current datasets in CAMK analysis:\n")
...
for (dataset_id in names(camk_results)) {
  ...
  if (is.list(dataset_info$groups)) {
    ...
    cat("   METHOD: Group detection successful\n")
    cat("   DATA: Pattern type:", pattern_type, "\n")
    cat("   NOTES: Column used:", column_used, "\n")
    
    if (length(groups) > 0) {
      group_table <- table(groups)
      cat("   SAMPLES: Groups found:\n")
      ...
      clinical_relevance <- "QUESTION: Unknown"
      
      if (pattern_type == "healthy_vs_disease" || pattern_type == "healthy_vs_disease_general") {
        comparison_type <- "SUCCESS: HEALTHY vs DISEASE"
        clinical_relevance <- "TARGET: HIGH - Direct disease relevance"
      } else if (pattern_type == "AF_SR") {
        comparison_type <- "WARNING: AF vs SR"
        clinical_relevance <- "WARNING: MODERATE - Disease subtype comparison"
        ...
        if (any(grepl("healthy|control|normal|non.failing", group_names, ignore.case = TRUE))) {
          ...
          comparison_type <- "ERROR: DISEASE vs DISEASE"
          clinical_relevance <- "ERROR: LOW - Not healthy vs disease"
        }
      }
      
      cat("   SEARCH: Comparison type:", comparison_type, "\n")
      cat("   RESULTS: Clinical relevance:", clinical_relevance, "\n")
      ...
      if (total_samples >= 50 && min_group_size >= 10) {
        power_assessment <- "SUCCESS: EXCELLENT - Good statistical power"
      } else if (total_samples >= 20 && min_group_size >= 5) {
        power_assessment <- "WARNING: MODERATE - Adequate for analysis"
      } else {
        power_assessment <- "ERROR: LOW - Limited statistical power"
      }
      
      cat("   DATA: Statistical power:", power_assessment, "\n")
      
      # CAMK gene availability
      if (!is.null(dataset_info$camk_genes_present)) {
        camk_count <- length(dataset_info$camk_genes_present)
        cat(sprintf("   GENETIC: CAMK genes: %d/11 available\n", camk_count))
      }
...
cat("TARGET: RECOMMENDATIONS FOR PROPER CAMK DYSREGULATION ANALYSIS:\n")
...
cat("TARGET: PRIMARY ANALYSIS (Healthy vs Disease):\n")
if (length(healthy_vs_disease) > 0) {
  cat("   SUCCESS: Recommended datasets:\n")
  ...
  cat("   ERROR: No healthy vs disease datasets identified!\n")
  cat("   FIX: Need to check GSE57338 - it should have healthy controls\n")
}

cat("\nWARNING: SECONDARY ANALYSIS (Disease Subtypes):\n")
if (length(disease_subtype) > 0) {
  cat("   WARNING: Disease subtype datasets (separate analysis):\n")
...
cat("\nERROR: EXCLUDED FROM MAIN ANALYSIS:\n")
...
cat("\nSUCCESS: Dataset verification completed!\n")