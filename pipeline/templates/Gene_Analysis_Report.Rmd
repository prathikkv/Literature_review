---
title: "Dynamic Pharmaceutical Research Report"
...
# Load configuration for dynamic content
library(yaml)
...
target_diseases <- config$research_target$diseases
...
# Load comprehensive analysis results (DYNAMIC)
load_analysis_results <- function() {
  tryCatch({
    # Load meta-analysis results (pipeline-relative paths)
    # Dynamic file path using gene name from config
    meta_file <- paste0("../output/current/", toupper(primary_gene), "_meta_analysis_FINAL.csv")
    meta_results <- read.csv(meta_file, stringsAsFactors = FALSE)
    
    # Load individual dataset results  
    # Dynamic file path for DGE results
    dge_file <- paste0("../output/current/", toupper(primary_gene), "_DGE_all_datasets_COMPREHENSIVE.csv")
    individual_results <- read.csv(dge_file, stringsAsFactors = FALSE)
...
# Extract primary gene results for highlighting (DYNAMIC)
primary_gene_meta <- analysis_data$meta_results[analysis_data$meta_results$Gene == primary_gene, ]

# Create dynamic alias for primary gene meta results (for backward compatibility)
camk2d_meta <- primary_gene_meta
```

```{r dynamic-title, results='asis'}
# DYNAMIC TITLE GENERATION (FIXED CONFIG INTEGRATION)
dynamic_title <- paste(primary_gene, "Gene Family in", paste(target_diseases, collapse = " and "))
cat("# ", dynamic_title, "\n\n", sep = "")
cat("**Comprehensive Meta-Analysis Report**\n\n")
cat("*Generated on ", format(Sys.Date(), "%B %d, %Y"), "*\n\n", sep = "")
cat("---\n\n")
```

```{r dynamic-intro, results='asis'}
# DYNAMIC INTRODUCTION (PROMPT 6)
cat("## Analysis Overview\n\n")
cat("This comprehensive analysis focuses on **", primary_gene, "** gene family in the context of ", 
    paste(target_diseases, collapse = " and "), ".\n\n", sep = "")

if (length(gene_family) > 0) {
  cat("**Gene Family Members Analyzed:** ", length(gene_family), " genes\n\n")
  cat("- Primary Gene: **", primary_gene, "**\n")
  if (length(gene_family) > 1) {
    family_members <- gene_family[gene_family != primary_gene]
    if (length(family_members) > 0) {
      cat("- Family Members: ", paste(family_members, collapse = ", "), "\n")
    }
  }
  cat("\n")
}

cat("**Diseases of Interest:**\n")
for (disease in target_diseases) {
  cat("- ", disease, "\n")
}

cat("\n**Analysis Parameters:**\n")
cat("- P-value threshold: ", analysis_params$p_value_threshold, "\n")
cat("- Log FC threshold: ", analysis_params$log_fc_threshold, "\n")
cat("- Minimum sample size: ", analysis_params$min_sample_size, "\n")
cat("- FDR correction: ", analysis_params$fdr_correction, "\n")
cat("- Meta-analysis method: ", analysis_params$meta_analysis_method, "\n\n")
```

<div class="nav-banner">
  <h4 style="margin: 0; font-weight: 600;">🧬 CAMK2D Pipeline Reports</h4>
  <p style="margin: 5px 0; opacity: 0.9;">Complete analysis and documentation suite</p>
  <div class="nav-links">
    <a href="Interactive_Technical_Documentation.html" target="_blank">
      📊 Interactive Documentation
    </a>
    <a href="#executive-summary">
      📈 Analysis Results
    </a>
    <a href="#meta-analysis">
      🔬 Meta-Analysis
    </a>
    <a href="#data-quality">
      ✅ Data Quality
...
```{r dynamic-executive-summary, results='asis'}
# DYNAMIC EXECUTIVE SUMMARY
cat("<div class='alert alert-success'>\n")
cat("**🎯 PRIMARY FINDING:** ", primary_gene, " shows significant differential expression in ", 
    paste(target_diseases, collapse = " and "), " studies.\n\n", sep = "")

if (nrow(primary_gene_meta) > 0) {
  cat("**📊 STATISTICAL RESULTS:**\n")
  cat("- Combined Effect Size (logFC): ", round(primary_gene_meta$Combined_logFC[1], 4), "\n")
  cat("- Meta-analysis P-value: ", format(primary_gene_meta$Combined_P_Value[1], scientific = TRUE), "\n")
  cat("- Statistical Significance: ", ifelse(primary_gene_meta$Significant[1], "✅ YES", "❌ NO"), "\n\n")
} else {
  cat("⚠️ No meta-analysis results found for ", primary_gene, "\n\n")
}

cat("**🔬 ANALYSIS SCOPE:**\n")
cat("- Primary Gene: **", primary_gene, "**\n")
cat("- Disease Context: ", paste(target_diseases, collapse = ", "), "\n")
if (length(gene_family) > 1) {
  cat("- Gene Family Size: ", length(gene_family), " members\n")
}
cat("</div>\n\n")
```

```{r dynamic-target-confirmation, results='asis'}
# DYNAMIC TARGET CONFIRMATION
cat("<div class='alert alert-success'>\n")
cat("**", toupper(primary_gene), " THERAPEUTIC TARGET ASSESSMENT**\n\n", sep = "")
cat("Our comprehensive meta-analysis across **", analysis_data$metrics$total_datasets, 
    " datasets** (", analysis_data$metrics$total_samples, " samples) ", sep = "")

if (nrow(primary_gene_meta) > 0 && primary_gene_meta$Significant[1]) {
  cat("confirms ", primary_gene, " as significantly dysregulated in ", 
      paste(target_diseases, collapse = " and "), " with strong therapeutic potential.", sep = "")
} else {
  cat("provides comprehensive analysis of ", primary_gene, " expression patterns in ", 
      paste(target_diseases, collapse = " and "), ".", sep = "")
}
cat("\n</div>\n\n")
```

```{r executive-summary}
# Create executive summary table (DYNAMIC)
if (nrow(primary_gene_meta) > 0) {
  executive_summary <- data.frame(
    ...
    Result = c(
      paste(primary_gene, if(primary_gene_meta$Significant[1]) "Significantly Dysregulated" else "Expression Analyzed"),
      paste0("p = ", sprintf("%.2e", primary_gene_meta$Combined_P_Value)),
      sprintf("%.4f", primary_gene_meta$Combined_logFC),
      paste0("[", sprintf("%.4f", primary_gene_meta$CI_Lower), " to ", sprintf("%.4f", primary_gene_meta$CI_Upper), "]"),
...
methodology_file <- "../output/current/methodology_comparison_analysis.csv"
if (file.exists(methodology_file)) {
  methodology_comparison <- read.csv(methodology_file, stringsAsFactors = FALSE)
} else {
...
alignment_summary <- if(nrow(methodology_comparison) > 0) {
  table(methodology_comparison$Alignment)
} else {
  table(character())
}

cat("**METHODOLOGY VALIDATION RESULTS:**\n")
if(length(alignment_summary) > 0) {
  ...
  cat("No methodology comparison data available\n")
}
...
if(nrow(methodology_comparison) > 0) {
  ...
  cat("No methodology comparison data available to display\n")
}
```
...
if (nrow(successful_datasets) > 0) {
  dataset_table <- successful_datasets %>%
    select(Dataset, Priority, Samples, Disease_Type, Biological_Context, CAMK_Genes_Analyzed, CAMK_Significant) %>%
    mutate(
      Success_Rate = paste0(round(CAMK_Significant/CAMK_Genes_Analyzed*100, 1), "%"),
  ...
  DT::datatable(dataset_table, 
    ...
    DT::formatStyle('Success_Rate',
                    background = DT::styleColorBar(dataset_table$CAMK_Significant, 'lightblue'))
}
```
...
primary_gene_individual <- analysis_data$individual_results[analysis_data$individual_results$Gene_Symbol == primary_gene, ]

# Create dynamic alias for backward compatibility
camk2d_individual <- primary_gene_individual

if (nrow(primary_gene_individual) > 0) {
  primary_gene_table <- primary_gene_individual %>%
  ...
  # Create dynamic alias for backward compatibility
  camk2d_table <- primary_gene_table
...
pathway_summary <- pathway_enrichments %>%
  ...
  rename(!!paste0(toupper(primary_gene), "_Genes_Present") := CAMK_Genes_Present) %>%
