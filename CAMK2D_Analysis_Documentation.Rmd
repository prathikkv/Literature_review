...
# Load real cardiovascular expression data
if (!file.exists("cache/comprehensive/processed_expression_data.rds")) {
  stop("❌ REAL DATA REQUIRED: Run the full pipeline first to generate processed_expression_data.rds")
}
...
expression_subset <- expression_matrix[1:min(10, nrow(expression_matrix)), 1:8]
...
camk_expression <- expression_matrix

# Create annotation with actual condition values
...
unique_conditions <- unique(sample_metadata$Condition)
condition_colors <- c("#2E86AB", "#A23B72")[1:length(unique_conditions)]
