---
title: "CAMK Analysis in Cardiovascular Disease"
subtitle: "Comprehensive CAMK2D-Focused Investigation: From Discovery to Clinical Translation (501 samples across 6 datasets)"
...
breakthrough_metrics <- data.frame(
  ...
  Current = c("501", "6", "6", "2", "2", "2", "2"),
  Improvement = c("1.6x", "6x", "6x", "2x", "2x", "2x", "New")
)
...
if (kable_enhanced) {
  dataset_summary %>%
    kable(caption = "Comprehensive Dataset Overview (Total: 501 samples)") %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
...
cat("• **Total Samples**: 501 (1.6x increase from original 313 single-dataset analysis)\n")
cat("• **Datasets**: 6 verified usable datasets (2 platforms)\n") 
cat("• **Species Coverage**: Human only (all verified datasets)\n")
cat("• **Data Types**: Microarray (5 datasets) + RNA-seq (1 dataset)\n")
cat("• **Primary Discovery**: GSE57338 (313 samples: 136 healthy + 177 disease)\n\n")
```
...
ggplot(comprehensive_datasets, aes(x = Resolution_Tier, y = Samples, fill = Data_Type)) +
  ...
  labs(
    ...
    subtitle = "501 total samples across 6 datasets",
    x = "Resolution Tier",
    y = "Number of Samples",
    fill = "Data Type"
  ) +
...
summary_stats <- data.frame(
  ...
  Key_Metrics = c(
    ...
    "501 samples, verified dataset quality"
  ),
...
cat("• **Total Samples Analyzed**: 501\n")
cat("• **Datasets Integrated**: 6\n")
cat("• **Analysis Framework**: Multi-dataset validation\n")
