...
subtitle: "End-to-End Demonstration and Documentation"
...
expression_matrix <- matrix(
  rnorm(n_samples * n_genes, mean = 6, sd = 2),
  nrow = n_genes,
  ncol = n_samples
)

# Add CAMK genes with differential expression
camk_indices <- 1:length(camk_genes)
for (i in camk_indices) {
  # Add differential expression in disease samples
  expression_matrix[i, 11:20] <- expression_matrix[i, 11:20] + rnorm(10, mean = 1.5, sd = 0.5)
}

rownames(expression_matrix) <- c(camk_genes, paste0("Gene_", (length(camk_genes)+1):n_genes))
colnames(expression_matrix) <- c(paste0("Control_", 1:10), paste0("Disease_", 1:10))

# Create sample metadata
sample_metadata <- data.frame(
  Sample_ID = colnames(expression_matrix),
  Condition = c(rep("Control", 10), rep("Disease", 10)),
  Dataset = "GSE_Example",
  Species = "human",
  row.names = colnames(expression_matrix)
)

cat("✅ Simulated expression data created:", nrow(expression_matrix), "genes x", 
...
# Perform simple differential expression (t-test for demonstration)
...
```{r forest-plot, fig.cap="Meta-Analysis Forest Plot for CAMK2D"}
# Simulate meta-analysis data for CAMK2D
meta_data <- data.frame(
  Study = c("GSE14975", "GSE31821", "GSE41177", "GSE79768", "GSE115574", "Pooled"),
  Effect_Size = c(0.62, 0.35, 0.78, 0.41, 0.55, 0.54),
  Lower_CI = c(0.25, 0.02, 0.41, 0.08, 0.22, 0.38),
  Upper_CI = c(0.99, 0.68, 1.15, 0.74, 0.88, 0.70),
  Weight = c(18, 22, 15, 20, 25, 100),
  N_Samples = c(10, 16, 19, 13, 29, 87)
)
