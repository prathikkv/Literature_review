...
if (nrow(processed_summary) > 0) {
  ...
  if (nrow(rnaseq_datasets) > 0) {
    ...
    kable(rnaseq_datasets %>% 
      ...
      kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
      row_spec(which(rnaseq_datasets$CAMK_Genes_Found == "10/10"), background = "#E8F5E8")
...
if (nrow(processed_summary) > 0) {
  ...
  detection_comparison <- tech_comparison %>%
    ...
    summarise(
      Datasets = n(),
      Perfect_Detection = sum(CAMK_Detection_Score == 10),
  ...
  kable(detection_comparison,
        ...
        col.names = c("Technology", "Total Datasets", "Perfect Detection (10/10)", "Avg CAMK Genes", "Perfect Detection Rate")) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
}
```
...
cat("- **Perfect CAMK detection**: 5/7 processed datasets (10/10 genes)\n\n")
